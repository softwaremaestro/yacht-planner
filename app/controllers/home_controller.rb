class HomeController < ApplicationController
  ...
  def index
    ...
    @newsfeeds = Newsfeed.order("updated_at desc").page(params[:page]).per(2).includes(:user,{:article => {:comments => :user}},:sub_category).where("plan_id in (#{ps.join(',')})")
    #includes(:user,{:article => {:comments => :user}},:sub_category).find_all_by_plan_id([ps])
